...
public class StaticPipelineConfigResolverTests extends ESTestCase {
    ...
    public void testIpFieldTypeUsesIpFieldBlockSize() {
        final StaticPipelineConfigResolver resolver = StaticPipelineConfigResolver.INSTANCE;
        final int formatBlockSize = randomFrom(128, 512);
        final FieldContext context = new FieldContext(formatBlockSize, randomNonTimestampFieldName(), null, null, MappedFieldType.IP, true);

        final PipelineConfig config = resolver.resolve(context);

        assertEquals(StaticPipelineConfigResolver.IP_FIELD_BLOCK_SIZE, config.blockSize());
    }

    public void testKeywordFieldTypeUsesKeywordFieldBlockSize() {
        final StaticPipelineConfigResolver resolver = StaticPipelineConfigResolver.INSTANCE;
        final int formatBlockSize = randomFrom(128, 512);
        final FieldContext context = new FieldContext(
            formatBlockSize,
        ...
        assertEquals(StaticPipelineConfigResolver.KEYWORD_FIELD_BLOCK_SIZE, config.blockSize());
    }

    public void testIpAndKeywordBlockSizeOverridesContextBlockSize() {
        final StaticPipelineConfigResolver resolver = StaticPipelineConfigResolver.INSTANCE;
        final int formatBlockSize = randomFrom(128, 512);

        final PipelineConfig ip = resolver.resolve(
            new FieldContext(formatBlockSize, randomNonTimestampFieldName(), null, null, MappedFieldType.IP, true)
        );
        final PipelineConfig keyword = resolver.resolve(
            new FieldContext(formatBlockSize, randomNonTimestampFieldName(), null, null, MappedFieldType.KEYWORD, true)
        );
        final PipelineConfig other = resolver.resolve(
            new FieldContext(formatBlockSize, randomNonTimestampFieldName(), null, null, null, false)
        );

        assertEquals(StaticPipelineConfigResolver.IP_FIELD_BLOCK_SIZE, ip.blockSize());
        assertEquals(StaticPipelineConfigResolver.KEYWORD_FIELD_BLOCK_SIZE, keyword.blockSize());
        assertEquals(formatBlockSize, other.blockSize());
    }

    public void testIpAndKeywordCachedConfigReturnSameInstance() {
        final StaticPipelineConfigResolver resolver = StaticPipelineConfigResolver.INSTANCE;

        final PipelineConfig firstIp = resolver.resolve(
            new FieldContext(randomFrom(128, 512), randomNonTimestampFieldName(), null, null, MappedFieldType.IP, true)
        );
        final PipelineConfig secondIp = resolver.resolve(
            new FieldContext(randomFrom(128, 512), randomNonTimestampFieldName(), null, null, MappedFieldType.IP, true)
        );
        assertSame(firstIp, secondIp);

        final PipelineConfig firstKeyword = resolver.resolve(
            new FieldContext(randomFrom(128, 512), randomNonTimestampFieldName(), null, null, MappedFieldType.KEYWORD, true)
        );
        final PipelineConfig secondKeyword = resolver.resolve(
            new FieldContext(randomFrom(128, 512), randomNonTimestampFieldName(), null, null, MappedFieldType.KEYWORD, true)
        );
        assertSame(firstKeyword, secondKeyword);
    }
